...
export default function ProductsPage() {
  ...
  return (
    <ProtectedShell>
      <div className="min-h-[calc(100vh-180px)] rounded-[2rem] bg-[#050505] p-4 text-white shadow-[0_24px_80px_rgba(0,0,0,0.22)] sm:p-10 lg:p-14">
        <div className="flex items-start justify-between gap-4 sm:gap-6">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-[#858585] sm:text-xs">Admin dashboard</p>
            <h1 className="mt-3 text-3xl font-semibold tracking-tight sm:mt-4 sm:text-6xl">Welcome back.</h1>
            <p className="mt-3 max-w-lg text-sm leading-6 text-[#999999] sm:mt-4 sm:text-base sm:leading-7">A clear view of your catalog, ready for the next update.</p>
          </div>
          <button type="button" aria-label="Open dashboard menu" onClick={() => setMenuOpen((value) => !value)} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-[#333333] text-[#BDBDBD] transition hover:border-[#777777] hover:text-white">
        ...
        <div className="mt-10 sm:mt-16">
          <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#777777] sm:text-xs">Catalog</p>
              <h2 className="mt-2 text-2xl font-semibold text-white">Product list</h2>
            </div>
            <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:items-center">
              <label htmlFor="product-search" className="sr-only">Search products</label>
              <input id="product-search" type="search" value={search} onChange={(event) => changeSearch(event.target.value)} placeholder="Search products..." className="w-full rounded-xl border border-[#333333] bg-[#101010] px-3 py-2 text-sm text-white outline-none placeholder:text-[#777777] focus:border-[#777777] sm:w-64" />
              <span className="text-sm text-[#777777] sm:inline">{total} products</span>
            </div>
          </div>

          <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            <label htmlFor="category-filter" className="sr-only">Filter by category</label>
            <select id="category-filter" value={category} onChange={changeCategory} className="w-full rounded-xl border border-[#333333] bg-[#101010] px-3 py-2.5 text-sm capitalize text-white outline-none focus:border-[#777777] sm:w-auto">
            ...
            <select id="sort-products" value={sortBy ? `${sortBy}:${order}` : ""} onChange={changeSort} className="w-full rounded-xl border border-[#333333] bg-[#101010] px-3 py-2.5 text-sm text-white outline-none focus:border-[#777777] sm:w-auto">
          ...
          {!error ? <div className="mt-8 flex flex-col items-center gap-4 text-sm text-[#999999]">
            <p className="text-center text-sm sm:text-base">Showing {firstItem}–{lastItem} of {total}</p>
            <div className="flex w-full flex-col items-center justify-center gap-3 sm:flex-row">
              <div className="flex w-full max-w-[360px] overflow-hidden rounded-2xl border border-[#333333] bg-[#101010] sm:w-auto">
                <button type="button" aria-label="Previous page" disabled={page === 1 || loading} onClick={() => goToPage(page - 1)} className="flex h-11 w-11 shrink-0 items-center justify-center border-r border-[#333333] text-2xl text-[#D0D0D0] transition hover:bg-[#1A1A1A] disabled:cursor-not-allowed disabled:opacity-40 sm:h-12 sm:w-12">‹</button>
                <div className="flex min-w-0 flex-1 justify-center">
                  {getVisiblePages(page, pageCount).map((pageNumber) => <button key={pageNumber} type="button" disabled={loading} onClick={() => goToPage(pageNumber)} className={`h-11 min-w-11 border-r border-[#333333] px-2 text-sm transition last:border-r-0 sm:h-12 sm:min-w-12 sm:px-3 sm:text-base ${pageNumber === page ? "bg-white text-black" : "text-[#D0D0D0] hover:bg-[#1A1A1A]"}`}>{pageNumber}</button>)}
                </div>
                <button type="button" aria-label="Next page" disabled={page === pageCount || loading} onClick={() => goToPage(page + 1)} className="flex h-11 w-11 shrink-0 items-center justify-center text-2xl text-[#D0D0D0] transition hover:bg-[#1A1A1A] disabled:cursor-not-allowed disabled:opacity-40 sm:h-12 sm:w-12">›</button>
              </div>
              <label htmlFor="page-size" className="sr-only">Products per page</label>
              <select id="page-size" value={pageSize} onChange={changePageSize} className="h-11 w-full rounded-xl border border-[#333333] bg-[#101010] px-3 text-sm text-white outline-none focus:border-[#777777] sm:h-12 sm:w-auto">
